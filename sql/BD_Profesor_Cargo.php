...
class BD_Profesor_Cargo extends BD_class_abstracta {
	...
	function stringConsultaTabla() {
		$sql="SELECT pc.id, pc.id_profesor, pc.id_cargo, p.apellido as apellidoprof, p.nombre as nombreprof, p.cuil, c.nombre as nombrecargo FROM profesor_cargo as pc, profesores as p, cargos as c WHERE pc.id_profesor=p.id AND pc.id_cargo=c.id";
